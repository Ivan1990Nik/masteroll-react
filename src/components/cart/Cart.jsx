...
function Cart() {
    ...
    const handleOrderSubmit = (e) => {
        ...
        const orderDetails = cart.map(item => {
            ...
            if (item.size) {
                // Предполагаем, что item.size — строка (например, "32cm") или объект; скорректируйте
                /* const sizeDisplay = typeof item.size === 'object' ? item.size.size : item.size; */
                details += `  размер: ${item.weight})`;
            } else if (item.weight) {
                details += ` (Вес: ${item.weight})`;
            }
            return details;
        }).join('\n');
                        ...
                        {cart.map(item => (
                            <div key={`${item.id}-${item.size}`} className="cart-item"> {/* УНИКАЛЬНЫЙ КЛЮЧ */}
                                ...
                                <div className="cart-item-details">
                                    ...
                                    <span className="cart-item-price">= {item.price * item.quantity} руб</span>
                                </div>
                                <button className="remove-btn" onClick={() => removeFromCart(item.id, item.size)}> {/* Передаём id И size */}
                                    Удалить
                                </button>
                            </div>
                        ))}
